...
|---|---|---|---|
| **Bolt** | Механический заклёпка | Действие, клик, активация | Кнопки, action-элементы |
...
### CSS prefix: `arc-`
```css
.arc-bolt-concave { }    /* Впуклая кнопка */
...
|---|---|---|---|---|---|
| 1 | **BoltButton** | `bolt_button/` | `.arc-bolt-` | concave, convex, radial, pill, ingot | 5 |
| 2 | **BoltRunic** | `bolt_runic/` | `.arc-bolt-runic-` | Рубленые кнопки (clip-path): single, double-side, double-full × 3 цвета | 9 |
| 3 | **BoltCombo** | `bolt_combo/` | `.arc-bolt-combo-` | Составная кнопка (icon + text): center, ltr | 2 |
| 4 | **BoltNav** | `bolt_nav/` | `.arc-bolt-nav-` | Навигационная иконка: circle/square × 6 стилей бордера | 12 |
...
|---|---|---|---|---|
| A | **ArcBentoHeader** | `arc_bento_header/` | CoreAvatar + FluxStatBar + FluxSegmentBar + EdgeGaltel + BoltNav | Flared Bento RPG Header |
| B | **ArcDialog** | `arc_dialog/` | BoltButton + EdgeDivider + RuneTitle | Пергаментный диалог квестов |
| C | **ArcPaperBoard** | `arc_paper_board/` | ArcSlip × N + RuneTitle | Деревянная доска объявлений |
| D | **ArcPopup** | `arc_popup/` | BoltButton + GearInput | Модальное окно |
...
| Concave / Convex buttons | BoltButton | ✅ |
| Shields (normal + double) | SigilShield | ✅ |
| Dialog V1 (simple) | ArcDialog | ✅ |
| Dialog V2 (quest + controls) | ArcDialog | ✅ |
| Radial button (diamond accents) | BoltButton (radial) | ✅ |
| Runic buttons (3 variants × 3 colors) | BoltRunic | ✅ |
| Diamond icons (hollow/filled) | SigilDiamond | ✅ |
| Pill + Ingot buttons | BoltButton (pill/ingot) | ✅ |
| Combo buttons (center/ltr) | BoltCombo | ✅ |
...
| Flared Menu Button | BoltNav (flared) | ✅ |
...
├── bolt_runic/          BoltRunic.js         + bolt_runic.css
...
2. `BoltButton` — 5 вариантов (concave/convex/radial/pill/ingot)
3. `BoltRunic` — 9 вариантов (3 стиля × 3 цвета)
4. `BoltCombo` — 2 варианта (center/ltr)
5. `BoltNav` — 12 вариантов (2 формы × 6 бордеров)
